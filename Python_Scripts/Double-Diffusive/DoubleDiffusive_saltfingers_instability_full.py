...
line1 = axis2.plot(Tloc,np.arange(len(Tloc)),'r')
line2 = axis2.plot(Sloc, np.arange(len(Sloc)),'b')
...
while solver.ok:
    ...
    GradT = np.mean(T['g'][:,-1]-T['g'][:,0])
    GradS = np.mean(S['g'][:,-1]-S['g'][:,0])

    newRrho = Rrho0*GradT/GradS
    problem.parameters['Rrho0'] = newRrho

    updateRho = problem.parameters['Rrho0']
