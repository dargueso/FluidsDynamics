#!/usr/bin/env python
'''
@File    :  StratifiedShearedFlow_KH_instability_viscous_diffusivity.py
@Time    :  2023/12/03 15:39:15
...
if run_model:
    ...
    problem.add_bc("left(rho) = 0.2")
    problem.add_bc("right(rho) = -0.2") #viceversa a la de R-T
    ...
    while solver.ok:
        ...
        if solver.iteration % 10 == 0:
            ...
            axis.set_title("Density t = %f" % solver.sim_time)
            fig.canvas.draw()
            plt.savefig(f"./StratifiedShearedFlow_KH_instability_viscous_diffusivity_{nt:03d}.png")
            nt += 1
